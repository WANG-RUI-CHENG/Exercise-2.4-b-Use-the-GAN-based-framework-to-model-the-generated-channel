...
os.environ["CUDA_VISIBLE_DEVICES"] = "1"  # 若只有 CPU 或單卡，可改成 '0' 或直接註解
...
def generate_real_samples_with_labels_Rayleigh(h_dataset, number=100):
    ...
    Steps:
        ...
        2. Generate random 16-QAM symbols.
        3. Create received signal y = h*x + n.
        4. Construct the conditioning vector.

    Args:
        h_dataset: Dataset of complex channel coefficients.
        number: Number of samples to generate.

    Returns:
        received_data: shape [number, 2], columns are [Re(y), Im(y)]
        conditioning: shape [number, 4], columns are [Re(x), Im(x), Re(h), Im(h)] / 3
    """
    # 1) Randomly sample channel coefficients h from dataset
    h_complex = np.random.choice(h_dataset, number)
    h_r = np.real(h_complex)
    h_i = np.imag(h_complex)

    # 2) Randomly generate transmitted 16-QAM symbols x
    labels_index = np.random.choice(len(mean_set_QAM), number)
    data = mean_set_QAM[labels_index]

    # 3) Simulate received signal y = h*x + n
    ...
    gaussian_random = np.random.multivariate_normal(
        mean=[0, 0], cov=[[0.01, 0], [0, 0.01]], size=number
    ).astype(np.float32)

    received_data = received_data + gaussian_random

    # 4) Construct conditioning vector [Re(x), Im(x), Re(h), Im(h)]
    conditioning = (
        np.hstack(
            (
...
D_solver = tf.train.AdamOptimizer(learning_rate=1e-4, beta1=0.5, beta2=0.9).minimize(
    D_loss, var_list=theta_D
)

G_solver = tf.train.AdamOptimizer(learning_rate=1e-4, beta1=0.5, beta2=0.9).minimize(
...
for it in range(750000):
    ...
    if (it + 1) % plot_every == 0:
        saver.save(sess, "./Models/ChannelGAN_model_step_" + str(it) + ".ckpt")
        ...
        for channel_idx in range(10):
            ...
            for idx in range(len(mean_set_QAM)):
                ...
                samples_component = sess.run(
                    G_sample,
                    feed_dict={
                        Z: sample_Z((number, Z_dim)),
                        Condition: conditioning.astype(np.float32),
                    },
                )
            ...
            plt.savefig(
                ...
                + "/"
                + str(channel_idx)
                + "_{}_noise_1.eps".format(str(i).zfill(3)),
                bbox_inches="tight",
            )
            plt.savefig(
                save_fig_path
                + "/"
                + str(channel_idx)
                + "_{}_noise_1.png".format(str(i).zfill(3)),
                bbox_inches="tight",
            )
        ...
        axes.set_ylim([-4, 4])
        plt.title(
            "Iter: {}, loss(D): {:2.2f}, loss(G):{:2.2f}".format(
                it + 1, D_loss_curr, G_loss_curr
            )
        )
        plt.savefig(
            save_fig_path + "/{}.png".format(str(i).zfill(3)), bbox_inches="tight"
        )
